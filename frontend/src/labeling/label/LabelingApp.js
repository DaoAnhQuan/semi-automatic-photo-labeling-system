...
import { List, Typography, Divider, Button, Menu } from "antd";
...
class LabelingApp extends Component {
  ...
  render() {
    ...
    const imageTab = this.props.review ? (
      <div
        style={{
          display: "flex",
          flex: 1,
      ...
      >
        <Menu
          mode="inline"
          defaultSelectedKeys={[`${this.props.image.id}`]}
          style={{ height: "100%" }}
        >
          <h3
            style={{
          ...
          {this.props.images.map((image) => {
            ...
            const likeStyle = reviewList[image.id] == 1 ? styles[1] : styles[0];
            const dislikeStyle =
              reviewList[image.id] == 0 ? styles[1] : styles[0];
            return (
              <Menu.Item
                onClick={() => {
                  this.props.redirectTo(image.id);
                }}
                key={image.id}
              >
                {/* Image: */}
                ...
                <DislikeOutlined
                  style={dislikeStyle}
                  onClick={() => {
                    if (reviewList[image.id] == 0) {
                      this.props.reviewImage(-1);
                      reviewList[image.id] = -1;
                      // this.setState({
                      //   reviewList,
                      // });
                      this.props.changeList(reviewList);
                    } else {
                      this.props.reviewImage(0);
                      reviewList[image.id] = 0;
                      // this.setState({
                      //   reviewList,
                      // });
                      this.props.changeList(reviewList);
