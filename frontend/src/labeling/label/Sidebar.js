    ...
    return (
      ...
      >
        <List
          divided
          selection
          style={{ flex: 1, overflowY: "scroll", maxHeight: "420px" }}
        >
          {labels.map((label, i) =>
            ListItem({
              ...
              label,
              color: colors[i],
              onSelect: !this.props.review && getSelectHandler(label),
        ...
        {!this.props.review && (
          <div style={{ position: "absolute", bottom: "10px", margin: "5px" }}>
            <Button
...
function ListItem({
  ...
  if (onToggle && typeHidable[label.type]) {
    icons.push(
      <Button
        key="visibility-icon"
        icon={isToggled ? <EyeOutlined /> : <EyeInvisibleOutlined />}
        onClick={(e) => {
          onToggle(label);
          e.stopPropagation();
        }}
